...
export class UserService {
  private baseUrl = 'http://localhost:3303/user';

  private followUrl='http://localhost:2033/follow'
  ...
  getFollowing(userid: number): Observable<Object>{
    return this.http.get(`http://localhost:2033/follow/following/`+`${userid}`)
  }

  getFollower(userid: number): Observable<Object>{
    return this.http.get(`http://localhost:2033/follow/follower/`+`${userid}`)
  }
